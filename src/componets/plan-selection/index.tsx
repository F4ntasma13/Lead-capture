...
//       if (response.ok) {
//         if (router) {
//           router?.push(`/feasibilityPage?plan=${encodeURIComponent(JSON.stringify(plan))}`);
//         }
//       } else {
...
const PlanSelection: React.FC = () => {
  ...
  // Recupera os dados enviados da página anterior
  ...
  const handleSelectPlan = async (obs: typeof plans[0]) => {
    // Combina os dados do plano com os dados anteriores
    const combinedData = {
      ...previousData,
      obs,
    };
